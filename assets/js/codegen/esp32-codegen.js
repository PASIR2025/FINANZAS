(function (global) {
  ...
  const VERSION = '1.8.0-autonomous-hmi-v24';
  ...
  function likelySafetyInput(label, source) {
    ...
    let metaText='';
    try { const vm=global.SimuPLCVariableManager,meta=vm&&vm.getMetadata?vm.getMetadata():null,item=meta&&meta.variables&&meta.variables[key];metaText=item?[item.symbolicName,item.name,item.description].filter(Boolean).join(' '):''; } catch (_) {}
    ...
    try {
      ...
      return els.some(function(el){if(!el)return false;const uses=[el.tag,el.tag2,el.tag3,el.tag4].some(function(v){return String(v||'').trim().toUpperCase()===key;});if(!uses)return false;const text=[el.label,el.name,el.description,el.reference,metaText].filter(Boolean).join(' ');return el.type==='estop'||critical.test(text)||(el.type==='pushbutton'&&el.contactType==='nc'&&/STOP|PARADA/i.test(text));});
  ...
  function generateFBDESP32(state, pinMap) {
    ...
    lines.push('  if(strcmp(cmd,"HELLO")==0){simuplcRemoteModeArmed=false;Serial.println(F("OK,SIMUPLC,READY_AUTONOMOUS_V24,1"));simuplcScanRequested=true;simuplcStateRequested=true;return;}');
