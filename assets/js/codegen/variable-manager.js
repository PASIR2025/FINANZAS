(function (global) {
  ...
  const VERSION = '1.7.0-v16-vars-v2';
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      @media(max-width:760px){
        ...
        .vm-table td:before{content:none}.vm-label{font-size:11px}.vm-status-dot{width:22px;height:22px}.vm-name,.vm-desc,.vm-pin{min-width:0;width:100%;height:31px;font-size:10px}.vm-row-message{max-width:none}
      ...
      html.theme-dark .vm-field select,html.theme-dark .vm-search,html.theme-dark .vm-name,html.theme-dark .vm-desc,html.theme-dark .vm-pin,html.theme-dark .vm-btn,html.theme-dark .vm-filter,html.theme-dark .vm-close{background:#0f172a;color:#e5eef8;border-color:#475569}
      html.theme-dark .vm-label{color:#f8fafc}html.theme-dark .vm-kind{color:#cbd5e1}html.theme-dark .vm-origin{background:#123149;color:#7dd3fc}html.theme-dark .vm-no-pin,html.theme-dark .vm-row-message{color:#aab9c8}
  ...
  function filteredRows(validation) {
    ...
    return currentRows.filter(function (row) {
      ...
      if (query) {
        const haystack = [row.label, row.symbolicName, row.typeTitle, row.origin, row.description, row.usage].join(' ').toLowerCase();
  ...
  function bindModalEvents() {
    ...
    document.getElementById('vmSave').addEventListener('click', function () {
      ...
      showToast(lastValidation && lastValidation.ok ? 'Variables, nombres y pines guardados' : 'Guardado; aún hay errores por corregir');
    });
  ...
  function exportCSV() {
    ...
    const profile = getProfile(currentBoard);
    const map = mapForBoard(currentBoard);
    const lines = [['Variable','Nombre','Tipo','Usado en','Descripción','Valor','Placa','Pin físico','Estado','Diagnóstico'].map(csvCell).join(',')];
    currentRows.forEach(function (row) {
      ...
      lines.push([
        row.label, row.symbolicName, row.typeTitle, row.origin, row.description, row.value, profile.name, pin,
